...
COMMENT ON COLUMN public.orders.delivery_fee IS 'Order shipping fee';
COMMENT ON COLUMN public.orders.total IS 'Order amount';
...
ALTER TABLE IF EXISTS public.orders ALTER id SET DEFAULT NEXTVAL('seq_orders');
...
CREATE SEQUENCE public.seq_order_items;
ALTER TABLE IF EXISTS public.order_items ALTER id SET DEFAULT NEXTVAL('seq_order_items');
